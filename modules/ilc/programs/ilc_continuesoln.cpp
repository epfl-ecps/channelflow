...
int main(int argc, char* argv[]) {
    cfMPI_Init(&argc, &argv);
    {
        ...
        const string muname = args.getstr("-cont", "--continuation", "",
                                          "continuation parameter, one of [Ra, Pr, gx, gz, Grav, Tref, P, Uw, UwGrav, "
                                          "Rot, Theta, ThArc, ThLx, ThLz, Lx, Lz, Aspect, Diag, Vs, VsNu, VsH, H]");
        ...
        if (restart) {
            ...
            uname =
                args.getstr(2, "<flowfield>", "velocity field of initial solution from which to start continuation");
            tname =
                args.getstr(1, "<flowfield>", "temperature field of initial solution from which to start continuation");
        }
        args.check();

        if (muname == "") {
            cerr << "Please choose --continuation with one of [Ra, Pr, gx, gz, Grav, Tref, P, Uw, UwGrav, Rot, Theta, "
                    "ThArc, ThLx, ThLz, Lx, Lz, Aspect, Diag, Vs, VsNu, VsH, H]"
                 << endl;
            exit(1);
        }
        args.save();
        ...
        dsi = unique_ptr<ilcDSI>(new ilcDSI());

        if (restart) {
            cout << "Restarting from previous solutions. Please be aware that DNSFlags and ILCFlags "
            ...
            cout << "Loading flags from " + restartdir[0] + "dnsflags.txt and " + restartdir[0] +
                        "ilcflags.txt, neglecting command line switches."
                 << endl;
            ilcflags.load(cfmpi->taskid(), restartdir[0]);
            dt = TimeStep(ilcflags);
            ...
            if (muname == "Aspect") {
                ...
                for (int n = 2; n >= 0; n -= 2) {
                    if (abs(diagonal - pythag(u[n].Lx(), u[n].Lz())) >= EPSILON) {
                        ...
                        temp[n].rescale(diagonal * cos(alpha_n), diagonal * sin(alpha_n));
                    }
                }
            } else if (muname == "Diag") {
                ...
                for (int n = 2; n >= 0; n -= 2) {
                    if (abs(aspect - u[n].Lx() / u[n].Lz()) >= EPSILON) {
                        ...
                        temp[n].rescale(diagonal_n * cos(alpha), diagonal_n * sin(alpha));
                    }
                }
            } else if (muname == "Lt") {
                ...
                for (int i = 2; i >= 0; i -= 2) {
                    if (abs(atan((Lztarg - u[i].Lz()) / (Lxtarg - u[i].Lx())) - phi) >= EPSILON) {
                        ...
                        temp[i].rescale(Lxtarg - mu[i] * cos(phi), Lztarg - mu[i] * sin(phi));
                    }
                }
            }
            ...
            project(ilcflags.symmetries, u[1], "initial value u", cout);
            project(ilcflags.tempsymmetries, temp[1], "initial value temp", cout);
            fixdivnoslip(u[1]);
        ...
        dsi = unique_ptr<ilcDSI>(new ilcDSI(ilcflags, sigma[0], 0, dt, Tsearch, Rxsearch, Rzsearch, Tnormalize,
                                            Unormalize, u[0], temp[0], N->getLogstream()));
        ...
        cout << "T     == " << T[0] << endl;
        cout << "dPdx  == " << ilcflags.dPdx << endl;
        cout << "dt    == " << dt.dt() << endl;
        cout << "ILCFlags == " << ilcflags << endl << endl;

        dsi->setPhaseShifts(xphasehack, zphasehack, uUbasehack);
        dsi->chooseMuILC(muname);
        if (!restart) {
            mu[1] = dsi->mu();
